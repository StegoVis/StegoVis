                    ...
                    <template v-for="action in actions">
                        <transition
                            ...
                            <template v-if="action.tooltip">
                                <li v-if="toggle" :style="{ 'background-color': action.color || bgColorOpen }"
                                    ...
                                    ref="actions">
                                    <span :class= "['icon', 'iconfont', actionIconSize, action.icon]"></span>
        ...
        <template v-if="rippleShow">
            <template v-if="mainTooltip">
                <div v-ripple="rippleColor == 'light' ? 'rgba(255, 255, 255, 0.35)' : ''" @click="toggle = !toggle"
                     ...
                     class="fab-main pointer" :style="{ 'background-color': toggle?bgColorOpen:bgColor, 'padding': paddingAmount }">
                     <span :class= "['icon', 'iconfont', mainIconSize, 'main', 'icon-folder', { rotate: toggle && allowRotation }]">
                     </span>
                    <span :class= "['icon', 'iconfont', mainIconSize, 'close', 'icon-folder-open', { rotate: toggle && allowRotation }]">
                     </span>
                </div>
            </template>
            <template v-else>
                <div v-ripple="rippleColor == 'light' ? 'rgba(255, 255, 255, 0.35)' : ''" @click="toggle = !toggle"
                    ...
                    <i :class="[ mainIconSize , { rotate: toggle && allowRotation }, 'material-icons close']">description</i>
                </div>
            </template>
        </template>
        <template v-else>
            <template v-if="mainTooltip">
                <div v-bind:v-tooltip="{ content: mainTooltip, placement: tooltipPosition, classes: 'fab-tooltip'}"
                    ...
                    <i class="material-icons md-36 close" :class="{ rotate: toggle && allowRotation }">description</i>
                </div>
            </template>
            <template v-else>
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            mainIcon: {
                default: 'folder_open' //folder_open
            },
            iconSize: {
                default: 'medium'
            },
            mainTooltip: {
                default: 'click to show all the inner document.'
            },
            fixedTooltip: {
                default: false
            },
        ...
        methods: {
            ...
            setPosition() {
                ...
                switch (this.position) {
                    case 'bottom-right':
                        this.pos.right = this.positionObj.leftRight;
                        this.pos.bottom = this.positionObj.topBottom;
                        break;
                    case 'bottom-left':
                        this.pos.left = this.positionObj.leftRight;
                        this.pos.bottom = this.positionObj.topBottom;
                        break;
                    case 'top-left':
                        this.pos.left = this.positionObj.leftRight;
                        this.pos.top = this.positionObj.topBottom;
                        break;
                    case 'top-right':
                        this.pos.right = this.positionObj.leftRight;
                        this.pos.top = this.positionObj.topBottom;
...
<style scoped>
    ...
    .fab-main .iconfont.main {
        opacity: 1;
        position: absolute;
    }

    .fab-main .iconfont.close {
        opacity: 0;
        position: absolute;
    }

    .fab-main .iconfont.main.rotate {
    ...
    .fab-main .iconfont.close.rotate {
        -ms-transform: rotate(360deg); /* IE 9 */
        -webkit-transform: rotate(360deg); /* Chrome, Safari, Opera */
        transform: rotate(360deg);
        opacity: 1;
